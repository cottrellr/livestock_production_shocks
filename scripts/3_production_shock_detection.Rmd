...
saveRDS(shocks_by_cooks_distance, file = here("data", "generated_data","shocks_to_primary_comms", "shocks_w_baselines_cooks_distance_COMMS.rds"))
...
shock_loess_apply <-   function(this_span){
  ...
  these_shocks <- 
    ...
    group_by(iso_3c, area, group, baseline_durations, average) %>% 
    nest() %>% 
    mutate(shock_presence = map(data, ~shock_detector_loess(target_time_series = .$value, target_cooks_distance = 0.35, target_span = this_span))) %>% 
    unnest(cols = c(data, shock_presence)) %>% 
...
saveRDS(shocks_by_span, file = here("data", "generated_data", "shocks_to_primary_comms", "shocks_w_baselines_span_COMMS.rds"))
